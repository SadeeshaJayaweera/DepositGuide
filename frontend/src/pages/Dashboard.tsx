...
export default function Dashboard() {
  ...
  const chartData = recommendation ? [
    { name: 'Baseline', value: recommendation.baseline_interest, color: '#ef4444' }, // Red
    { name: 'Recommended', value: recommendation.projected_interest, color: '#10b981' } // Green
  ] : [];

  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <header className="mb-10">
        <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">Your Financial Overview</h1>
        <p className="text-slate-400 text-lg">AI-powered insights for your latest statement.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-800/80 rounded-2xl p-6 border border-slate-700/50 shadow-lg backdrop-blur-xl relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
              <div className="flex items-center gap-4 mb-4 text-slate-400">
                <div className="p-3 rounded-xl bg-slate-900/80 shadow-inner">
                  <CreditCard size={20} className="text-indigo-400" />
                </div>
                <h3 className="font-semibold tracking-wide uppercase text-sm">Statement Balance</h3>
              </div>
              <p className="text-3xl font-bold text-white">LKR {statement_summary.balance.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
            </div>
            ...
            <div className="bg-slate-800/80 rounded-2xl p-6 border border-slate-700/50 shadow-lg backdrop-blur-xl relative overflow-hidden group">
              ...
              <p className="text-3xl font-bold text-white">{new Date(statement_summary.due_date).toLocaleDateString()}</p>
            </div>
            ...
            <div className="bg-slate-800/80 rounded-2xl p-6 border border-slate-700/50 shadow-lg backdrop-blur-xl relative overflow-hidden group">
              ...
              <p className="text-3xl font-bold text-rose-300">LKR {statement_summary.minimum_payment.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
            </div>
          </div>

          {/* Deposit Recommendation */}
          {recommendation && (
            <div className="bg-slate-800/90 rounded-3xl p-8 border border-slate-700 shadow-2xl backdrop-blur-xl relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div className="space-y-4">
                  ...
                  {Object.entries(recommendation.schedule).map(([date, amount]) => (
                    <div key={date} className="flex justify-between items-center bg-slate-900/50 p-4 rounded-xl border border-slate-700/50 hover:border-emerald-500/30 transition-colors">
                ...
                <div className="h-64 w-full bg-slate-900/30 rounded-2xl p-4 border border-slate-700/30">
        ...
        <div className="space-y-8">
          ...
          {/* Explainability */}
          <div className="bg-slate-800/60 rounded-3xl p-6 border border-slate-700/50 backdrop-blur-md">
            ...
            <div className="space-y-4">
              {explanations.map((exp, i) => (
                <div key={i} className="group">
                  <h4 className="text-slate-200 font-medium mb-1 group-hover:text-indigo-300 transition-colors">{exp.line_item_name}</h4>
                  <p className="text-slate-400 text-sm leading-relaxed">{exp.plain_language_explanation}</p>
                  {i < explanations.length - 1 && <div className="h-px bg-slate-700/50 mt-4"></div>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="bg-slate-800/60 rounded-3xl p-6 border border-slate-700/50 backdrop-blur-md">
              ...
              <div className="space-y-4">
                {low_value_subscriptions.map((sub, i) => (
                  <div key={i} className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50 border-l-4 border-l-amber-500">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-slate-200 font-medium">{sub.name}</h4>
                      <span className="text-amber-400 font-bold text-sm">LKR {sub.amount.toLocaleString()}</span>
                    </div>
